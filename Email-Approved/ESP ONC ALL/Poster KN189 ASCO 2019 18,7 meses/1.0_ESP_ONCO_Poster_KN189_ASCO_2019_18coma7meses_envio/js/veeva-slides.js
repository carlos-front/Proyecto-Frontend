...
com.usacd.slides = (function () {
	...
	function initCustomSwipe()
	{
		...
		if(body.dataset.swipeNext || document.body.dataset.swipePrev || document.body.dataset.swipeDown || document.body.dataset.swipeUp || iVerticalTotalPages > 0 || iHorizontalTotalPages > 0)
		{
			...
			{
				try{ Slide.aPages[iVerticalPage].onEntry();}catch (e){}
			}
			// < Vertical pages
			// Horizontal pages >
			...
			function endSwipe(poE)
			{
				...
				if((iAbsDeltaX > 50 || iAbsDeltaY > 50) && (Date.now() - iTime < 1000))
				{
					poE.stopPropagation();
					// Horizontal swipe
					...
					{
						if(iDeltaY > 0 && iVerticalPage < iVerticalTotalPages)
						{
							try{ Slide.aPages[iVerticalPage].onExit(1);}catch (e){}
							iVerticalPage++;
							hVerticalSection.style.webkitTransform = 'translateY(' + (iVerticalPage * -768) + 'px)';
							hVerticalSection.addEventListener(TRANSITION_END, onEntryVerticalPage);
						}
						else if(iDeltaY < 0 && iVerticalPage > 0)
						{
							try{ Slide.aPages[iVerticalPage].onExit(-1);}catch (e){}
							iVerticalPage--;
							hVerticalSection.style.webkitTransform = 'translateY(' + (iVerticalPage * -768) + 'px)';
